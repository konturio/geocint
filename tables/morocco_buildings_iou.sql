...
create table morocco_buildings_manual_extent as (
    ...
    from morocco_buildings_manual b
             join morocco_buildings_benchmark_extents a
                  on ST_Intersects(ST_MakeValid(b.footprint), ST_MakeValid(a.wkb_geometry))
);
...
update morocco_buildings_benchmark_extents
...
create index on morocco_buildings_benchmark_extents using gist (footprint);
...
set max_height = (
    select max(building_height)
    from morocco_buildings_benchmark_extents b
