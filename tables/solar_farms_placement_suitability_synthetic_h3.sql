...
with gsa_ghi as (select gsa.h3                                  as h3,
     ...
     slope as (select gebco.h3 as                                                     h3,
                      case
                          when gebco.avg_slope_gebco_2024 < 0 then 1
                          when gebco.avg_slope_gebco_2024 > 4 then 0
                          else (1 - ((gebco.avg_slope_gebco_2024 - 0) / (4 - 0))) end slope
               from gebco_2024_h3 gebco),
...
-- constraints
     ...
     constraint_slope as (select gebco.h3 as    h3,
                                 case
                                     when gebco.avg_slope_gebco_2024 < 5 then 1
                                     -- set values from 1 to 0.1 by proportion
                                     when gebco.avg_slope_gebco_2024 < 6.8 
                                         then (7 - gebco.avg_slope_gebco_2024) * (0.5)
