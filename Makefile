...
tile_generator/bin: tile_generator/main.go tile_generator/go.mod
	cd tile_generator; go get; go build -o bin
...
data/tiles/basemap_all: tile_generator/bin db/function/basemap_mapsme db/table/water_polygons_vector db/table/osm2pgsql | data/population/population_api_tables.sqld.gz data/tiles
	tile_generator/bin -j 32 --min-zoom 0 --max-zoom 8 --sql 'select basemap($1, $2, $3)' --db-config 'host=localhost dbname=gis' output-path data/tiles/basemap/
	touch $@
