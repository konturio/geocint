...
create table zero_pop_h3 as (
    ...
    where
        exists(select from osm_water_polygons w where ST_Intersects(p.geom, w.geom))
);

update kontur_population_mid1 p set has_water = true, probably_unpopulated = true from zero_pop_h3 z where z.h3 = p.h3;
