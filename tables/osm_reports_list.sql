...
select 'population_tag_check',
        ...
        'Find discrepancies between [Kontur Population](https://www.kontur.io/portfolio/population-dataset/) and [OpenStreetMap](https://wiki.openstreetmap.org/wiki/Key:population) data to see potential errors in OpenStreetMap population data. [Kontur Population](https://data.humdata.org/dataset/kontur-population-dataset) is a global population dataset generated based on various public data sources including OpenStreetMap. Inconsistencies in the values between Kontur data and OpenStreetMap population key on administrative division boundaries may indicate inaccuracies in OSM data. Click to download [https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_reports@@@@@/population_check_osm.csv](https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_reports@@@@@/population_check_osm.csv)',
        true,
        {1},
        true
from osm_meta;
...
insert into osm_reports_list (id, name, link, last_updated, description_brief, description_full, searchable_columns_indexes, public_access)
select 'osm_population_inconsistencies',
        ...
        'This report indicates potential errors in OpenStreetMap population key values. [Kontur](https://www.kontur.io/) analyzes boundary objects to check if the population numbers on different levels of administrative boundaries do not conflict with each other. For example, a country’s population may not be smaller than the sum of its region’s population. Click to download [https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_reports@@@@@/osm_population_inconsistencies.csv](https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_reports@@@@@/osm_population_inconsistencies.csv)',
        false,
        {1},
        true
from osm_meta;
...
select 'osm_gadm_comparison',
        ...
        'Compare [GADM](https://gadm.org/data.html) and OpenStreetMap statistics on administrative division to find potential gaps in OSM data. GADM boundary data is a global dataset with an administrative division of all countries. Its [license](https://gadm.org/license.html) forbids importing it into OpenStreetMap but this report by [Kontur](https://www.kontur.io/) allows you to analyze the hierarchy of boundaries in GADM and OpenStreetMap. The last column shows the number of subregions for every admin boundary found in GADM/OSM to make potential conclusions. GADM is not the [''ground truth''](https://wiki.openstreetmap.org/wiki/Ground_truth), but a big difference in stats is a good reason to look carefully into OSM data. Click to download [https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_reports@@@@@/osm_gadm_comparison.csv](https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_reports@@@@@/osm_gadm_comparison.csv)',
        false,
        {2},
        true
from osm_meta;
...
select 'osm_unmapped_places',
        ...
        'A list of viewed on [OpenStreetMap](https://www.openstreetmap.org) but unmapped places where people live according to [Kontur Population](https://data.humdata.org/dataset/kontur-population-dataset). Click to download [https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_reports@@@@@/osm_unmapped_places.csv](https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_reports@@@@@/osm_unmapped_places.csv)',
        false,
        {0},
        true
from osm_meta;
...
select 'osm_missing_roads',
        ...
        'A list of places with roads missing while comparing with [Facebook](https://github.com/facebookmicrosites/Open-Mapping-At-Facebook). Click to download [https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_reports@@@@@/osm_missing_roads.csv](https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_reports@@@@@/osm_missing_roads.csv)',
        true,
        {2},
        true
from osm_meta;
...
select 'osm_missing_boundaries_report',
        ...
        'A list of boundaries that disappeared from Kontur Boundaries dataset nightly version compared to the [last public one](https://data.humdata.org/dataset/kontur-boundaries). In most cases this means that the boundary polygon can not be extracted because relation in OpenStreetMap is broken and needs to be repaired. Click to download [https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_reports@@@@@/osm_missing_boundaries_report.csv](https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_reports@@@@@/osm_missing_boundaries_report.csv)',
        true,
        {3}
        false
from osm_meta;