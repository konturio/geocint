...
with zoom_levels as (select generate_series(0, 12) zoom),
     previous_outputs_raw as (select (calculate_h3_res_old(zoom_levels.zoom)).zoom_lvl        as zoom_lvl,
                                     (calculate_h3_res_old(zoom_levels.zoom)).tile_resolution as h3_res
                              from zoom_levels),
     previous_outputs as (select distinct zoom, h3_res
                          ...
                          order by zoom),
     new_outputs as (select zoom, calculate_h3_res(zoom, hex_edge_pixels := 44) as h3_res from zoom_levels),
