...
alter table covid19_us_counties_in
    add column admin_id int;
update covid19_us_counties_in a
set
    admin_id = b.admin_id
from
...
insert into covid19_log (date, admin_id, confirmed, dead, population)
    select
        a.time_value as date,
        a.admin_id,
        coalesce(a.value, 0) as confirmed,
        coalesce(b.value, 0) as dead,
        d.population
    from
        covid19_us_counties_in              a
        left join covid19_us_counties_in    b on a.date = b.date and a.admin_id = b.admin_id
