...
data/out/reports/osm_gadm_comparison.csv: db/table/osm_gadm_comparison | data/out/reports ## Export OSM-GADM comparison report to CSV with semicolon delimiter.
	psql -qXc "copy (select case when left(\"OSM name\", 4) = '   -' or \"OSM name\" is null then 'subrow_' else '' end || coalesce('href_[' || \"OSM id\" || '](https://www.openstreetmap.org/relation/' || \"OSM id\" || ')', '') as \"OSM id\", \"Admin level\",'hrefIcon_[' || case when left(\"OSM name\", 4) = '   -' or \"OSM name\" is null then 'tab_' else '' end || \"OSM name\" || '](http://localhost:8111/load_object?new_layer=false&objects=r' || \"OSM id\" || '&relation_members=true)' as \"OSM name\", case when left(\"OSM name\", 4) = '   -' or \"OSM name\" is null then 'tab_' else '' end || \"GADM name\" as \"GADM name\" from osm_gadm_comparison order by id limit 10000) to stdout with (format csv, header true, delimiter ';');" > $@
...
data/out/reports/osm_population_inconsistencies.csv: db/table/osm_population_inconsistencies | data/out/reports ## Export population inconsistencies report (see also db/table/osm_population_inconsistencies target) to CSV with semicolon delimiter.
	psql -qXc "copy (select case when left(\"Name\", 3) = ' - ' then 'subrow_' else '' end || coalesce('href_[' || \"OSM ID\" || '](https://www.openstreetmap.org/relation/' || \"OSM ID\" || ')', '') as \"OSM id\", 'hrefIcon_[' || case when left(\"Name\", 3) = ' - ' then 'tab_' else '' end || \"Name\" || '](http://localhost:8111/load_object?new_layer=false&objects=r' || \"OSM ID\" || '&relation_members=true)' as \"Name\", \"Admin level\", \"Population\", \"Population date\", \"Population source\", \"SUM subregions population\", \"Population difference value\", \"Population difference %\" from osm_population_inconsistencies order by id) to stdout with (format csv, header true, delimiter ';');" > $@
...
data/out/reports/osm_unmapped_places.csv: db/table/osm_unmapped_places_report | data/out/reports ## Export report to CSV
	psql -qXc "copy (select \"Kontur population\", \"osm.org view count\", 'hrefIcon_[Edit in JOSM](http://localhost:8111/load_and_zoom?' || \"Place bounding box\" || ')' as \"Place bounding box\" from osm_unmapped_places_report) to stdout with (format csv, header true, delimiter ';');" > $@

data/out/reports/osm_missing_roads.csv: db/table/osm_missing_roads | data/out/reports ## Export report to CSV
	psql -qXc "copy (select \"Country\", \"OSM roads length, km\", \"Facebook roads length, km\", 'hrefIcon_[Edit in JOSM](http://localhost:8111/load_and_zoom?' || \"Place bounding box\" || ')' as \"Place bounding box\" from osm_missing_roads where \"Country\" in ('Saint Lucia', 'Romania', 'Albania') and diff > 3.5 order by diff desc) to stdout with (format csv, header true, delimiter ';');" > $@
