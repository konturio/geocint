...
data/out/reports/osm_missing_roads.csv: db/table/osm_missing_roads | data/out/reports ## Export report to CSV
	psql -qXc 'copy (select "Country", "OSM roads length, km", "Facebook roads length, km", "Place bounding box" from osm_missing_roads where diff > 3 order by diff desc limit 10000) to stdout with (format csv, header true, delimiter ";");' > $@
