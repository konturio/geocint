...
create temporary table osm_pop_stats as (
  select
    percentile_cont(0.33333) within group (order by count)      as count_ab,
    percentile_cont(0.66666) within group (order by count)      as count_bc,
    1::float                                                    as population_12,
    percentile_cont(0.66666) within group (order by population) as population_23
  from
...
create table osm_quality_bivariate_grid_1000 as (
  select
    ...
    count,
    case
      when count <= count_ab then 'A'
      when count <= count_bc then 'B'
      else 'C' end ||
    case
      when population <= population_12 then '1'
      when population <= population_23 then '2'
