...
select s.h3                                                                                                                              as "H3 index",
       ...
       'left=' || ST_XMin(ST_Envelope(ST_Transform(s.geom, 4326))) || '&right=' || ST_XMax(ST_Envelope(ST_Transform(s.geom, 4326))) ||
       '&top=' || ST_YMax(ST_Envelope(ST_Transform(s.geom, 4326))) || '&bottom=' || ST_YMin(ST_Envelope(ST_Transform(s.geom, 4326)))     as "Place bounding box"
from stat_h3 s
