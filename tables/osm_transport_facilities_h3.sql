...
create table osm_transport_facilities_h3 as (
    select h3_lat_lng_to_cell(ST_PointOnSurface(geom)::point, 8)              as h3,
           ...
           nullif(count(*) filter (where type = 'car_parking'), 0)            as osm_car_parkings_count,
...
call generate_overviews('osm_transport_facilities_h3', '{osm_airports_count,osm_railway_stations_count,osm_public_transport_stops_count,osm_car_parkings_count}'::text[], '{sum,sum,sum,sum}'::text[], 8);
