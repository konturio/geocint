...
COLUMNS = (
    'latitude', 'longitude', 'brightness', 'bright_ti4',
    'scan', 'track', 'satellite', 'instrument',
    'confidence', 'version', 'bright_t31', 'bright_ti5',
    'frp', 'daynight', 'acq_datetime',
)
CHUNK_SIZE = 10000
...
def main(filename):
    ...
    with pd.read_csv(filename, dtype=str, chunksize=CHUNK_SIZE) as reader:
        for chunk in reader:
            ...
            # Calculate hash
            chunk['hash'] = chunk.apply(
                lambda x: md5(''.join(map(str, tuple(x))).encode("utf-8")).hexdigest(),
                axis=1
            )
