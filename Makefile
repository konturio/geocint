...
db/table/facebook_roads: data/mid/facebook_roads/extracted db/table/osm_roads | db/table ## loading files into the db
...
data/out/reports/osm_gadm_comparison.csv: db/table/osm_gadm_comparison | data/out/reports ## Export OSM-GADM comparison report to CSV with semicolon delimiter.
	psql -qXc "copy (select case when subrow is true then 'subrow_' else '' end || coalesce('href_[' || osm_id || '](https://www.openstreetmap.org/relation/' || osm_id || ')', '') as \"OSM id\", admin_level as \"Admin level\", 'hrefIcon_[' || case when subrow is true then 'tab_' else '' end || osm_name || '](http://localhost:8111/load_object?new_layer=false&objects=r' || osm_id || '&relation_members=true)' as \"OSM Name\", case when subrow is true then 'tab_' else '' end || gadm_name as \"GADM Name\" from osm_gadm_comparison order by id limit 10000) to stdout with (format csv, header true, delimiter ';');" > $@
...
data/out/reports/osm_population_inconsistencies.csv: db/table/osm_population_inconsistencies | data/out/reports ## Export population inconsistencies report (see also db/table/osm_population_inconsistencies target) to CSV with semicolon delimiter.
	psql -qXc "copy (select case when subrow is true then 'subrow_' else '' end || coalesce('href_[' || osm_id || '](https://www.openstreetmap.org/relation/' || osm_id || ')', '') as \"OSM id\", 'hrefIcon_[' || case when subrow is true then 'tab_' else '' end || \"name\" || '](http://localhost:8111/load_object?new_layer=false&objects=r' || osm_id || '&relation_members=true)' as \"Name\", admin_level as \"Admin level\", population as \"Population\", population_date as \"Population date\", population_source as \"Population source\", sum_subregions_population as \"SUM subregions population\", population_difference_value as \"Population difference value\", population_difference_percent as \"Population difference %\" from osm_population_inconsistencies order by id) to stdout with (format csv, header true, delimiter ';');" > $@
...
data/out/reports/osm_unmapped_places.csv: db/table/osm_unmapped_places_report | data/out/reports ## Export report to CSV
	psql -qXc "copy (select population as \"Kontur population\", view_count as \"osm.org view count\", 'hrefIcon_[Edit in JOSM](http://localhost:8111/load_and_zoom?' || bbox || ')' as \"Place bounding box\" from osm_unmapped_places_report order by id) to stdout with (format csv, header true, delimiter ';');" > $@

data/out/reports/osm_missing_roads.csv: db/table/osm_missing_roads | data/out/reports ## Export report to CSV
	psql -qXc "copy (select country as \"Country\", osm_roads_length_km as \"OSM roads length, km\", facebook_roads_length_km as \"Facebook roads length, km\", 'hrefIcon_[Edit in JOSM](http://localhost:8111/load_and_zoom?' || bbox || ')' as \"Place bounding box\" from osm_missing_roads where country in ('Saint Lucia', 'Romania', 'Albania') and diff > 3.5 order by diff desc) to stdout with (format csv, header true, delimiter ';');" > $@
