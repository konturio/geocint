...
drop table if exists testcity_geom;
create table testcity_geom as (
    select ST_Buffer(geom::geography,1609.3)::geometry geom
    from testcity
);

create index on testcity_geom using gist(geom);

-- extract data for county area
drop table if exists testcity_stat_h3_r10;
create table testcity_stat_h3_r10 as (
    select s.*,
    ...
    from (select h3_cell_to_children(s.h3, 10) as h3, 
          ...
          from testcity_geom g left join land_polygons_h3_r8 s on ST_Intersects(s.geom, ST_Transform(g.geom,3857))
         left join gebco_h3 o on (s.h3 = o.h3)
...
drop table if exists testcity_kp_h3r11;
create table testcity_kp_h3r11 as (
    select k.* 
    from kp_h3r11 k, 
         testcity_geom g 
...
update testcity_stat_h3_r10
set population = k.population
from testcity_kp_h3r11 k
where k.h3 = testcity_stat_h3_r10.h3;
...
alter table testcity_stat_h3_r10 add column nareas integer;
drop table if exists testcity_count;
create table testcity_count as (
    select h3,
           count(*)
    from testcity_stat_h3_r10 g, 
         areas_of_concern c 
...
update testcity_stat_h3_r10
set nareas = g.count
from testcity_count g
where g.h3 = testcity_stat_h3_r10.h3;
...
-- alter table testcity_stat_h3_r10 add column fires_count integer;
-- update testcity_stat_h3_r10 set fires_count = g.fires_count from testcity_historical_fires_h3_r10 g where g.h3 = testcity_stat_h3_r10.h3;
...
alter table testcity_stat_h3_r10 add column cost float;

-- implement cost calculation with mcda based on stat_h3 indicators
drop table if exists testcity_cost;
create table testcity_cost as (
    select h3,
           ...
           coalesce((avg_slope_gebco - min(avg_slope_gebco) OVER ()) / (max(avg_slope_gebco) OVER () - min(avg_slope_gebco) OVER ()), 0) +
...
update testcity_stat_h3_r10
set cost = t.cost
from testcity_cost t
where t.h3 = testcity_stat_h3_r10.h3;

-- remove temporary tables
drop table if exists testcity_cost;
drop table if exists testcity_count;

create index on testcity_stat_h3_r10 using gist(geom);
...
-- create temporary copy of testcity_stat_h3_r10 table
drop table if exists testcity_stat_h3_r10_copy;
create table testcity_stat_h3_r10_copy as (
    select h3, 
           cost, 
...
create index on testcity_stat_h3_r10_copy using gist(geom);

-- calculate costs for each pole
drop table if exists testcity_poles_with_calculated_cost;
create table testcity_poles_with_calculated_cost as (
...
drop table if exists testcity_poles_ranked;
create table testcity_poles_ranked as (
    select id   as id, 
           cost as cost_source, 
           priority,
           cost, 
    ...
    from testcity_poles_with_calculated_cost
);
create index on testcity_poles_ranked using btree(cost);
create index on testcity_poles_ranked using gist(geom);
...
declare 
    ...
    cur_pole record;
begin 
    while 0 < (select count(*) from testcity_poles_ranked where rank is null and cost > 0) loop
        raise notice 'Counter %', counter;

        -- store current pole in cur_pole variable
        select * into cur_pole
            from testcity_poles_ranked
            where rank is null
            order by cost desc limit 1;

        -- zero out surrounding hexagons
        update testcity_stat_h3_r10_copy g
        set cost = 0
        where ST_Intersects(cur_pole.geom,g.geom);

        -- set pole rank
        update testcity_poles_ranked
        set rank = counter
        ...
        from testcity_poles_ranked g inner join
             (select n.id,
                     sum(t.cost) as updated_cost
              from testcity_poles_ranked n,
                   testcity_stat_h3_r10_copy t
              where ST_Intersects(n.geom, t.geom)
                    ...
                    group by n.id) as sq
        on g.id = sq.id
        where testcity_poles_ranked.id = sq.id;
...
create table poles_for_sensors_placement as (
    select g.objectid,
           ...
           g.geom 
    from testcity_poles_with_calculated_cost g,
         testcity_poles_ranked t
    where g.id = t.id
);
...
-- enrich testcity_stat_h3_r10 table with updated costs, to keep information about ares
-- that weren't covered with sensors
alter table testcity_stat_h3_r10 add column updated_cost numeric;
update testcity_stat_h3_r10
set updated_cost = g.cost
from testcity_stat_h3_r10_copy g
where testcity_stat_h3_r10.h3 = g.h3;
