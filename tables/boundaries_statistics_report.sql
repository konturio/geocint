...
create table boundaries_statistics_report_mid as (
	select s.osm_id,
	       ...
	       sum(s.populated_area_km2 * (1 - sign(s.count)))                                                                as populated_area_with_no_osm_objects_km2,
	       sum(s.populated_area_km2 * (1 - sign(s.building_count)))                                                       as populated_area_with_no_osm_buildings_km2
...
create table boundaries_statistics_report as (
	select 
            ...
            trunc(cast(c.populated_area_with_no_osm_objects_km2 as numeric), 3)   as "Populated area with no OSM objects km2",
            trunc(cast(c.populated_area_with_no_osm_buildings_km2 as numeric), 3) as "Populated area with no OSM buildings km2",
            trunc(cast(c.roads_unmapped_km2 as numeric), 3)                       as "Populated area with no OSM roads km2",
