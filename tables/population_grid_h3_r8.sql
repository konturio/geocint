...
create table population_grid_h3_r8 as (
    ...
    from (
             select h3, population as ghs_pop, null::float as hrsl_pop, null::float as fb_africa_pop,
                    null::float as fb_pop
             from ghs_globe_population_grid_h3_r8
             union all
             select h3, null::float as ghs_pop, population as hrsl_pop, null::float as fb_africa_pop,
                    null::float as fb_pop
             from hrsl_population_grid_h3_r8
             union all
             select h3, null::float as ghs_pop, null::float as hrsl_pop, population as fb_africa_pop,
                    null::float as fb_pop
             from fb_africa_population_grid_h3_r8
             union all
             select h3, null::float as ghs_pop, null::float as hrsl_pop, null::float as fb_africa_pop,
                    population as fb_pop
...
update population_grid_h3_r8 p
set fb_africa_pop = 0
from fb_africa_population_boundary b
where ST_Intersects(ST_Transform(b.geom, 4326), p.geom)
